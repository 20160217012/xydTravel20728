...
public class BaseController {
	...
	//注入一个MessagesSource对象
	@Autowired
	protected MessageSource messageSource;
	
	protected final static String BASE_PATH = "http://localhost:8080/travel-user-web";
	//请求参数的验证，统一的处理方法
	@ExceptionHandler(BindException.class)
	public String bindException(BindingResult bind,HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public void authenticationException(HttpServletRequest request, HttpServletResponse response) {
		if (isAjaxRequest(request)) {
			...
			map.put("status", "666");
			map.put("message", "您尚未登录或登陆已超时，请重新登陆！");
			writeJson(map, response);
		}else {
			try {
				request.getSession().setAttribute("logMsg", "您尚未登录或登陆已超时，请重新登陆！");
	...
	protected String upLoadFile(HttpServletRequest request, MultipartFile multipartFile, String empNumber) {
		try {
